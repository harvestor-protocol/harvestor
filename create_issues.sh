...
# Issue 1: Score boundary validation tests
gh issue create \
  --title "test: Add score boundary validation tests for score-attestation contract" \
  --label "type:test,complexity:trivial,good-first-issue" \
  --body "## Description
...
The \`submit_score\` function validates that score <= 100. We need tests for boundary cases.
...
gh issue create \
  ...
  --label "type:docs,complexity:trivial" \
  --body "## Description
...
gh issue create \
  ...
  --label "type:feature,complexity:medium" \
  --body "## Description
...
4–8 hours (medium complexity: storage management, timestamp logic)"
...
gh issue create \
  ...
  --label "type:test,complexity:medium" \
  --body "## Description
...
4–8 hours (medium: testnet setup, CLI interaction, documentation)"
...
4–8 hours (medium: writing, testing commands, organizing)"

echo "✅ All 8 issues created on GitHub!"
